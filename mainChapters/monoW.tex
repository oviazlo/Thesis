...
The total efficiency (product of acceptance and reconstruction efficiency)
needed by the statistical analysis to calculate the number of expected events is shown in \FigureRef{fig:AccEff_mu}.
The slow decrease of the total efficiency from 1 TeV can be explain of the shape
of the $\PWprime$ signal, which is become more prolonged to the low-$mT$ region with increasing of $\PWprime$ pole mass as shown in \FigureRef{???}.

\begin{figure}[]
  \centering
  \includegraphics[width=0.65\textwidth]{Wprime/acceptance.eps}
  \caption{
  Total reconstruction efficiency ($\varepsilon_{tot}$) of the $\PWprime \to \mu \nu$ as a function of simulated $\PWprime$ mass.
  }
  \label{fig:AccEff_mu}
\end{figure}

Statistical analysis demonstrates that no excess larger than 2$\sigma$ is observed.
This is why the upper limit on the cross section for the production of $\PWprime$ times branching ratio has been set. 
...
Lower mass limits of the model correspond to the intersection of the theoretical curve with the expected cross section limit. The obtained mass limits are summarized in \TableRef{tab:limits_mass_wp}. The mass limit are for 800 GeV stronger in comparison with previous ATLAS search reported in ref.~\cite{wprime_8TeV}.
...
Beside the $\PWprime$ model discussed above, there are other BSM models 
with lepton plus $E_{T}^{miss}$ final state signature.
One of such models is an associated 
production of pair of weakly interacting massive particles (WIMP)
with SM W boson. WIMPs are one among of the possible dark matter (DM) candidates.
...
Because WIMPs don't interact strongly or electromagnetically they will most probably escape detection the same way as neutrinos
from leptonic W decays, and will contribute to $E_{T}^{miss}$ of the event.

In this section, a qualitative study based on models recommended by the Dark Matter Forum (link?) are presented, 
which addresses the question about sensitivity of signal selection presented in \SectionRef{sec:wprimeSelection} to such kind of models.
...
There are plethora of models which try to introduce and explain DM as hypothetical particles which can be produced at LHC. 
All these models can be classified to the three distinct classes: DM Effective Field Theories (EFT), Simplified DM models 
and Complete DM models. The EFT approach allows one to describe the DM-SM interactions mediated by all 
kinematically inaccesible particles in a universal way. 
% TODO say something about \Lambda scale. What does it means or what does it define.
%      is it the scale up to which gauge symmetry works?
It allows to derive stringent bounds on the ``new physics'' scale $\Lambda$. 
Simplified models are charachterized by the most important state mediating the DM interaction with SM. Unlike the EFT approach,
...
s- or t-channel exchange. Complete DM models allow to describe correlations between observables~\cite{arXiv:1506.03116}.

Main focus of the current study will stay in EFT and Simplified DM models with W boson and DM particles produced in the final state. 
In the simplified model, W is produced as an initial state radiation from one of the incoming quarks, as shown in \FigureRef{fig:feynMonoWSimple}. 
In the EFT approach, W can be produced as a final state particle (\FigureRef{fig:feynMonoWEFT}) or an initial state radiation (\FigureRef{fig:feynMonoWEFT}).
...
\toDo[explain shaded area!].
}
  \label{fig:feynMonoWEFT}
\end{figure}

Current study covers both s- and t-channel simplified models with different mass of $Z'$ mediator and with different mass of DM particles.
EFT approach is represented by two models which correspond to diagrams shown in \FigureRef{fig:feynMonoWEFT}. 
% TODO explan \Lambda or M*
They are characterized by the energy scale of the ``new physics'' $\Lambda$ (or effective mass $M^{*}$), as well as the mass of produced DM particles.
Both models assume vector type of interaction between DM and SM sectors. 
% TODO this is crap written. rewrite it!!!
For the D52 model ref~\cite{???} (which corresponds to the right diagram in  \FigureRef{fig:feynMonoWEFT}) 
we consider only constructive interference with SM.

\subsection{Sensitivity studies}
% TODO Oxana comment: Explain what exactly did you do?

The main idea of the study is to understand kinematics of the DM models in the final state with one lepton and missing energy and to estimate sensitivity 
of the signal selection to such kind of models.
The signal selection is designed with focus of high-$m_{T}$ region in order to get rid of the dominant SM W background.
Kinematic distributions of the DM models were studied to evaluate contribution to the signal selection and to make comparison
with the signal from the $\PWprime$ model.
It's worth to mention that due to the fact that the mass of the DM particles doesn't affect kinematic distribution of the events, 
all the simulated DM samples used are with the mass of the DM particles set to 1 GeV.
(should some plots to be shown to proof this???).

In \FigureRef{fig:kinematicsSChannel} the normalized kinematic distributions of the transverse mass of a lepton and $E_{T}^{miss}$ of the event
for the s-channel simplified model as well as EFT models with comparison with $\PWprime$ model are shown.
First distinguishable characteristics of all DM models is that there is no clear peak structure in any kinematic distribution, 
which is expected because transverse missing energy is formed by
neutrino from W boson decay and two DM particles which are independent from each other. 
Second feature of the DM models is that main contribution tend to be in low-$m_{T}$ region.
Especially it concerns simplified models, for which the dominant contribution is outside of the signal region for any parameter of mediator mass.
But with increasing of the mediator mass $m_{T}$, distribution tends to become more flat and moves towards high-$m_{T}$ region.
Also, with increasing mediator mass, the cross section of the process is significantly decreasing (see \TableRef{tab:TriggerDetails}) 
and for mass of 10 TeV, the cross section is a few orders of magnitude lower than the background \toDo[proof?], which 
makes signal selection not sensitive for the s-channel simplified DM model.
...
\caption{Normalized kinematic distributions of the transverse mass (left) and transverse missing energy in the event (right) of the simplified model in the s-channel, as well as EFT models compared to the $\PWprime$ distribution.}
...
But the cross sections for the t-channel processes are one-two orders of magnitude lower when compared to the s-channel (see \TableRef{tab:TriggerDetails}), which leads to the conclusion that signal selection is even less sensitive for the t-channel simplified model. Also, SM W background are a few orders higher in the entire range of $m_{T}$ than any sample of the simplified model.
For the D52 EFT model, an excess over SM W backgound can be seen in high-$m_{T}$ region, while for WW$\chi\chi$ EFT model it is not the case, because the cross section is very low.
...
\caption{Kinematic distributions of the transverse mass of the simplified model in the s- and t-channels (left), 
and EFT models with the $\PWprime$ signal (right) in comparison with the SM W background scaled to the respective process cross section.}
  \label{fig:scaledKin}
\end{figure}
...
A search for a new heavy gauge boson $\PWprime$ has been performed in the final state of muon and transverse missing energy using 3.2 fb$^{-1}$ of 13 TeV center of mass pp collision data collected in 2015. 
Upper limit on the cross section for SSM $\PWprime$ production have been set as a function of $\PWprime$ pole mass. A significantly stronger exclusion mass limit have been obtained in comparison with previous ATLAS searches~\cite{atlas_7tev_pub_1fb,atlas_7tev_pub,wprime_8TeV} as shown in \FigureRef{fig:Wprime_allResults}.
...
There are different ways to improve and develop the analysis.
...
Other improvement mentioned before is to use $m_T$ binned diboson and top MC background samples, which will provide enough statistics to populate high-$m_T$ region and no extrapolations will be needed (which is the dominant contribution to the systematic uncertainty).

% muon identification cut
Also one can investigate the possibility to loose a strict ``high-$p_T$'' set of muon identification criteria, which reduce the efficiency of muon reconstruction for approximately 20$\%$ in comparison with ``tight'' one. However one need to be careful about the muon momentum resolution not to worsen significantly.

% new methods from SM W measurements
The better understanding of the missing transverse mass could bring a significant reduction of the systematic uncertainty in the low-$m_T$ region, which is used for validation of the fake background estimation.

Also sensitivity study of the signal selection for different DM model which predict production of WIMPs with association with SM W has been done.
It has been shown that current signal selection has low sensitivity to considered DM models
moreover, is not sensitive as searches with jet final signatures.
