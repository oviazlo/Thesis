...
The invariant mass and transverse mass distributions for the $\PWprime$ samples with pole masses of 2, 3, 4 and 5 TeV are shown superimposed on top of SM $W$ background in \FigureRef{fig:signal_with_W}.
...
To get sufficient statistics at high transverse mass, several samples, binned in invariant mass of the lepton-neutrino pair, are used and are shown in \FigureRef{fig:wenu_notstacked}.
...
In this case, it will be not used in the $E_T^{miss}$ calculation itself, but will contribute to the $E_T^{miss}$, as will be described in \SectionRef{subsec:etmiss}.
...
% TODO Monika: ``Also the distributions'' --> ``Also regions''?
...
The validation distributions of criteria variables used to define the enhanced ``fake'' muon control region, are shown in \FigureRef{fig:muMMval1}.
...
The distribution of the $E_T^{miss}$, muon momentum and $m_T$ are shown in \FigureRef{fig:muMMval2}.
...
% TODO some more discussion here!
...
The systematic variations on the ``real'' and ``fake'' efficiency used in the matrix method were
...
of the $\PWprime$ signal, which becomes more prolonged in the low-$mT$ region with increasing of $\PWprime$ pole mass as shown in \FigureRef{???}.
\toDo[insert plot]
...
\toFix[I am still rewriting this section. I removed the text to avoid confusion.]
...
A search for a new heavy gauge boson $\PWprime$ has been performed in the final state of a muon and transverse missing energy using 3.2 fb$^{-1}$ of 13 TeV center of mass pp collision data collected in 2015. 
Upper limits on the cross section for SSM $\PWprime$ production have been set as a function of $\PWprime$ pole mass. A significantly stronger exclusion mass limit has been obtained in comparison with previous ATLAS searches~\cite{atlas_7tev_pub_1fb,atlas_7tev_pub,wprime_8TeV} as shown in \FigureRef{fig:Wprime_allResults}.
...
% TODO
% TODO uncomment, when you will uncomment part about mono-W study
% TODO
...
% It has been shown that the current signal selection has low sensitivity to considered DM models
