...
The second source comes from the wrong identification of the lepton itself, when a pion or a jet is reconstructed as a lepton, or from leptons which were produced 
...
This source also includes events in which a prompt electron which emits a photon due to hard bremsstrahlung. Subsequently the photon creates an electron-positron pair in which one of the two leptons receives most of the energy.
...
This table summarizes which MC generators and parton distribution function (PDF) sets were used, and the order of cross section calculations.
...
An event selected for the analysis have at least one reconstructed vertex with at least three tracks matched to it. 
...
Events should have at least two electron candidates and fire the dilepton trigger, which requires the presence of two electrons with $p_T > 12$ GeV.
...
where $\eta_e$ and $\phi_e$ are rapidity and azimuthal angle of the electron candidate and $\eta_i$ and $\phi_i$ are rapidity and azimuthal angle of track or calorimeter cluster, not related to the electron candidate. Detailed explanations of the isolation and other requirements on electron candidates are given later in this thesis.
...
\begin{itemize}
 \item 
 \item $p_T > 20$ GeV: to ensure a high and constant trigger efficiency as a function of $p_T$ and to harmonizes $p_T$ requirement between the three analysis channels ($ee$, $\mu\mu$ and $e\mu$) 
 \item $1.37<|\eta|$ or $1.52<|\eta|<2.47$: to be within high-granularity acceptance of the EM calorimeter, but excluding barrel-end-cap transition region.
 \item Electron tracks have to originate from the primary vertex.
 Transverse impact parameter significance, which is defined as the ratio of the absolute transverse impact parameter ($d_0$) to its uncertainty parameters ($\sigma(d_0)$) have to be below three. The distance between the $z$-coordinate of the primary vertex and $z$-position of the point of closest approach of the electron track in the ID to the beamline to be less than 1 mm.
 ...
 This set provide the best background rejection, a factor of two with respect to the medium selection, 
...
Electrons in the pair are classified by the $p_T$: the electron with a higher $p_T$ is called leading lepton, while the another one is the subleading lepton.
The leading lepton has to pass a more strict cut of $p_T>25$ GeV, while the subleading lepton has to satisfy $p_T>20$ GeV as described above.
...
Because the prompt background is relatively small in the signal region, a possibility of a charge misidentification cannot be neglected and has to be precisely estimated. 
...
In order to estimate how many events with opposite-sign dileptons will contribute to the same-sign dilepton signal region, 
...
By using generator level information one can distinguish charge misidentification due to photon conversion from other effects which is shown in the figure.
...
This method is found to be the most precise method due to using most statistics available and providing kinematically unbiased results~\cite{anthony_thesis} 
...
The charge misidentification scale factor are also applied for the MC prediction of the background from $W\gamma$ process
% TODO Monika comment
as the origin of creating a same-sign electron pair is very similar.
In this case an electron is produced in the $W$-decay and the $\gamma$ conversion.
...
that the electron only has to satisfy ``medium'' set of electron identification criteria~\cite{electron_tight} instead of the ``tight'',
and it has to fail the calorimeter- or track-based isolation criterion, or both.
$N_{\mathrm{P}}^{\mathrm{prompt}}$ and $N_{\mathrm{F}}^{\mathrm{prompt}}$ are the numbers of real prompt
leptons, obtained from MC simulation, which pass the signal and looser selections respectively.
...
which means that in each of these regions only one background type has to give a dominant contribution with respect to all other backgrounds.
...
\TableRef{tab:dilep_isoOS} gives the observed and expected number of the electron pairs.
...
In order to check the normalization of these processes, a prompt validation region is used.
...
the invariant mass $70 < m_{\Plepton\Plepton}< 110$ GeV region have been excluded
...
The $p_T$ and $\eta$ distribution for the leading electron are shown in \FigureRef{fig:charge_flip_CR_kinematics}.
...
The observed and expected numbers of electron pairs are also shown in \TableRef{tab:ee_isoSS_Z} for all same-sign electron pairs and separately for positively and negatively charged pairs.
...
except either the weaker identification requirement (shown as ``VR1''), and 
...
as well. Such a design of the validation region provide similar kinematics to the one in the signal region.
...
but fail the ``tight'' electron identification requirement, while passing ``medium''.
...
Electron reconstruction and ``tight'' identification efficiencies are obtained with so-called tag-and-probe data-driven methods.
This method allows to measure the efficiency of a studied electron selection using $Z \to e e$ and $J/\psi \to e e$ resonance decays from data. 
...
The limited number of simulated events in the Monte Carlo samples lead to an additional uncertainty which is listed as ``MC statistics'' in \TableRef{tab:syst}.
...
The background prediction describes the observed numbers of all of these distributions reasonable well within the uncertainties bands.
...
The fiducial volume represents the phase space region that is truncated in a manner that mimics the detector acceptance.
...
But because the real detector does not provide a 100$\%$ reconstruction and identification efficiency, in order to relate the fiducial volume with the real geometrical detector acceptance
...
The obtained efficiencies are in the range of 48-74$\%$. The lowest efficiency was observed for the fourth generation down-type quark model, while the highest one is found for the heavy right-handed $W_R$ boson and heavy Majorana neutrino process. The latter model has a larger efficiency with respect to the doubly charged Higgs model
...
The green and yellow bands correspond to the 1$\sigma$ and 2$\sigma$ uncertainty bands on the expected limits respectively.}
...
for events with $e^{+}e^{+}$ (left) and $e^{+}e^{+}$ (right) pairs. The green and yellow bands correspond to the 1$\sigma$ and 2$\sigma$ uncertainty bands on the expected limits respectively.}
...
Since the final state of this model has two same-sign pairs of leptons, no jet activity or no missing transverse energy is present in the event. 
...
The kinematics of left- and right-handed Higgs bosons is identical, but the production rate is different due to a different coupling to the Z boson mediator~\cite{dch_note}. The cross sections were calculated with NLO precision. 
...
Due to the limited statistics of the predicted background, one cannot have too many mass bins, otherwise the cross section
...
% TODO short summary about presented results
...
% TODO outlook
Since 13 TeV results for the inclusive search have not been public yet, 
results on the double charged Higgs have been already
one can compare mass limit to follow the improvement of the results with using data with higher center of mass energy and higher luminosity collected.

% improvement with center of mass energy and collected luminosity
One of the distinctive results of the analyses are mass limit on the \dch.
95\% C.L. upper \dch mass limits, assuming 100\% branching ratio to $e^{\pm}e^{\pm}$ 
compared with 7 TeV~\cite{dch_7TeV_paper} and 13 TeV~\cite{dch_13TeV_conf} results are shown in \TableRef{tab:DCH_limit_vs_years}.

One of the possibility to increase sensitivity of the search is try to optimize signal region to more efficiently reject charge misidentification background.


% improvement methods
As one compare these three analyses one can notice that the dominant sources of the systematic uncertainties arises from modelling of the charge misidentification and non-prompt backgrounds. Thus, improving method used for these backgrounds could allow to reduce the systematic uncertainty. 
