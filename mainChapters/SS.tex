...
\toAsk[is it OK to use ``truth information''?]
...
Other background which is present after the signal selection is so the called non-prompt background.
...
described before in \SectionRef{subsec:electron_selection} and $N_{\mathrm{F}}$ is number of electron do not meet 
...
lepton, obtained from MC simulation, which pass signal and looser selections and have to be subtracted in order to make sure that fake factor was calculated with using only non-prompt electrons \toDo[weird phrasing, rewrite!].

Due to large enough statistics of the background region the fake factor is measured as a function of 
electrons $E_\mathrm{T}$ and $\eta$. Knowing fake factor allows to calculate the total number of the events,
$N_{\mathrm{NP}}$, with same-sign lepton pairs, passed signal selection, but which contain non-prompt leptons by formula:
\toAsk[how to rephrase it?]
\begin{equation}
...
\label{eq:fake_pred}
\end{equation}
where first term correspond to the number electron pairs ($N_{P_l+F_s}$), 
where leading electron (denoted by ``l'') pass selection criterea ($P_l$) and subleading electron (denoted by ``s'')
fail to fulfils it ($F_s$). Contribution of every such pair to the signal region is scaled by fake factor 
$f_{\mathrm{s}}(p_{\mathrm{Ti}},|\eta_{i}|)$, where $p_\mathrm{Ti}$ and $\eta_{i}$ is transverse momentum and $\eta$
of subleading electron from pair which fails selection. Similarly second term represent number of pairs where
leading electron fails selection while subleading one pass it. Last term correspond to the case when both leading
and subleading electrons fail signal selection and it has to be subtracted from the sum to correct for the double
...
\includegraphics[width=0.48\textwidth]{SS/support_note/electrons/electron_fake_plot.eps}
\caption{Electron fake factor versus $E_\mathrm{T}$ with full statistical and systematic errors.}
\label{fig:ff_e_errs}
\end{center}
\end{figure}
