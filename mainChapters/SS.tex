...
Despite a great success of the Standard Model (SM) to describe interaction of the elementary particles there are plethora phenomena
which cannot be explained by the SM. Gravity, Dark Energy and Matter, neutrino masses and other evidence is not predicted by the
SM, which indicate its incompleteness.
This is why many studies at LHC experiments are focused on probing new energetic regimes to search of new physics, 
observation of which can prove or deny different extensions of the SM.

Many beyond the Standard Model (BSM) models predict same-sign dilepton final state,
which is very distinctive final state.
Also it is a very clean signal due to very few SM processes, which predict such final state.
This lead to high new physics sensitivity.

There are number searches in ATLAS which target the same final signature but all of them targets some given 
BSM model which means requiring additional cuts on number of jets \toFix[link] of missing transverse energy \toFix[link].
Current analysis is aim to be as inclusive as possible and to probe exclusively same-sign dilepton signature and
all the BSM models which predict it.

For these purpose signal selection consists from the minimal kinematic requirements on leptons.
Electron are very clean object which can be precisely reconstructed with high efficiency in ATLAS.
The search observable for the analysis was chosen to be same-sign dilepton invariant mass.

If no physics is found an upper limit on fiducial cross section for new physics can be set.
As an example of BSM model with same-sign dilepton final state, a pair production of double charged Higgs bosons
will be studied in details within current search.

Analysis use three channels: $e^{\pm}e^{\pm}$, $\mu^{\pm}\mu^{\pm}$ and $e^{\pm}\mu^{\pm}$.
This chapter will describe search only with $e^{\pm}e^{\pm}$ channel.
...
\end{center}
  ...
  The upper part of the table contains MC sample which provide same-sign dilepton in the final state.
...
\toDo[no jet selection is described]

The analysis is based on the pp data collected in 2012 by the ATLAS detector with 8 TeV center of mass energy.
Integrated luminosity of the data correspond to 20.3 fb$^{-1}$, the mean number of interactions per bunch crossing equal to 21.

Event should have at least one reconstructed vertex with at least three tracks matched to it. If there are few vertices, the one with the highest
...
which fire dilepton trigger, which require presence of two electrons with $p_T > 12$ GeV.
In order to exclude ambiguities between electron and jet reconstruction electron are required to
be isolated. Isolation is calculated by summing up activity around the electron candidate within a given cone size, $\Delta R = \sqrt{ (\eta_e-\eta_i)^2 + (\phi_e-\phi_i)^2 }$,
where $\eta_e$ and $\phi_e$ are rapidity and azimuthal angle of the electron candidate and $\eta_i$ and $\phi_i$ are rapidity and azimuthal angle of track or calorimeter cluster, 
not related to the electron candidate.
...
\begin{itemize}
 ...
 \item No reconstructed jet within $\Delta R < 0.4$. To remove ambiguous classification of the track, when track is classified as electron and as jet simultaneously. 
...
\caption{Electron charge misidentification rate, obtained from MC simulation with using electrons from Z boson decay. 
Contribution from charge flip due to photon conversion is shown separately.}
...
\end{subfigure}
\caption{Electron charge misidentification rate, obtained from MC simulation using truth (generated) information and likelihood method.}
...
\end{subfigure}
\caption{Electron charge misidentification rate, obtained from MC simulation and from real collision data with likelihood method.}
...
The electron fake factor as a function of electrons $p_T$ with statistical and systematic errors are
...
\caption{Electron fake factor $f$ as a function of electron $p_{T}$. Combined statistical and systematic errors shown with error bars.}
\label{fig:ff_e_errs}
\end{center}
\end{figure} 
...
\caption{Invariant mass of the opposite-sign electron pairs passed signal selection.}
\label{fig:OS_CR}
\end{center}
\end{figure} 
...
\caption{Prompt background validation region.}
\label{fig:prompt_CR}
\end{center}
\end{figure} 
...
\caption{Invariant mass of reconstructed same-sign electron pairs arising due to electron charge flip identification. Validation region for the charge flip background prediction.}
...
\end{subfigure}
\caption{\toDo Leading electron $p_T$ and $\eta$ distributions in the same-sign $Z$ peak validation region with two isolated electrons.}
...
Shaded region in the lower plot correspond to the combination of the statistical and systematic uncertainties of the background prediction.}
...
\end{subfigure}
\caption{$p_T$ and $\eta$ distributions of the leading electron in the signal region.}
...
\caption{95\% C.L. upper limits on the fiducial cross section for new physics contributing to the signal region for events with $e^{\pm}e^{\pm}$ pairs.}
...
\end{subfigure}
\caption{95\% C.L. upper limits on the fiducial cross section for new physics contributing to the signal region
for events with $e^{+}e^{+}$ (left) and $e^{+}e^{+}$ (right) pairs.}
  \label{fig:signal_kinematics}
\end{figure}
...
\caption{\toDo Total reconstruction efficiency ($\varepsilon_{tot}$) of the doubly charged Higgs bosons decay to same-sign electron pair as a function of simulated $H^{\pm\pm}$ mass, 
fitted with piecewise empirical function.}
...
\toAsk[CLs = CLs+b/CLb. What is the signal in case of limit on new physics?]
...
\caption{\toDo 95\% C.L. upper limits on the cross section for new physics contribution for events with $e^{\pm}e^{\pm}$ pairs.  
