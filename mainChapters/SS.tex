...
% The same-sign \Zboson\ peak with two isolated electrons (defined as $80 < \mee < 100$~GeV) is used to test the charge flip prediction, with kinematics shown in Figure~\ref{fig:ee_2iso_SS_Z}.  
% %The disagreement in $\eta$ seen in the opposite-sign control regions in Figure~\ref{fig:ee_2iso_OS} is not clearly visible here.
% 
% %For these plots only, the alternate fake denominator (``interiso'') described in section~\ref{fake_e_qflip} is used, as the mass shape of the fake prediction for the nominal denominator is distorted.  
% Table~\ref{tab:ee_isoSS_Z} shows the expected and observed number of events, using the fake prediction from the nominal method.  
% %The fake predictions for the nominal and interiso denominators are compared in Table~\ref{tab:ssZ_e_qflipcor} and agree well after the charge flip correction (by construction).
% 
% Results are also shown in Table~\ref{tab:ee_isoSS_Z} separately for $e^{+}e^{+}$ and $e^{-}e^{-}$ pairs.  
% %A slight charge asymmetry is observed in this region, $1.12\pm0.06$ for $e^{+}e^{+}/e^{-}e^{-}$ in data.  This is well modeled by the MC in this region, where the charge asymmetry predicted is $1.17\pm0.06$.

\begin{figure}
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=\textwidth]{SS/support_note/ChargeFlipCR/ElEl/ElEl_selection_pt_lead_ee_SS_CF_CR.eps}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=\textwidth]{SS/support_note/ChargeFlipCR/ElEl/ElEl_selection_eta_lead_ee_SS_CF_CR.eps}
\end{subfigure}
...
\begin{table}[htbp]
\caption{Observed and expected number of lepton pairs for the control region with same-sign, isolated electrons falling inside the $Z$ mass window. The uncertainties on the predictions are combined statistical then systematic \toDo[is it the case? or it is only statistical errors?].}
\begin{center}
\begin{tabular}{l|c}
\hline
...
%
% \hline \hline
% \multicolumn{2}{c}{\textbf{Same-sign $e^{+}e^{+}$ \Zboson\ mass window.}} \\\hline 
...
% 
% Control regions enhanced in fake electrons are defined for the $ee$ channel to test the fake predictions.
% Four different  control regions are defined for pairs of like-sign electrons, 
% either relaxing the isolation requirement or by using electrons that 
% fail "isEM tight++" reconstruction requirement, while passing "isEM medium++".
% 
% \begin{itemize}
% \item Like-sign electron pairs, both passing the signal region isolation requirement, but fails "isEM tight++", while passing "isEM medium++".
% \item Like-sign electron pairs where both electrons fails the signal isolation requirement but pass a looser \textit{intermediate} isolation cut instead.
% \item Like-sign electron pairs. The leading $\pt$ electron must pass the signal isolation cut, while the subleading electron fails the signal region isolation and instead passes the looser intermediate isolation cut.  
% \item Like-sign electron pairs. The subleading $\pt$ electron must pass the signal isolation cut, while the leading electron fails the signal region isolation and instead passes the looser intermediate isolation cut.  
% \end{itemize}
% 
% Table~\ref{tab:ee_fakeCR} shows the expected and observed numbers of electron pairs for the fake-enhanced control regions. 
% The uncertainties quoted are statistical only. For the uncertainty on the fake predictions, 
...
% Figure~\ref{fig:ee_fakeCR_mll} shows the invariant mass distributions for the four control regions. 
% The agreement between observation and prediction is generally good.  
% 
% Additional plots of fake-enhanced control regions can be found in Appendix~\ref{app:eeCR}. 
% These include $\pt$ spectra and $\eta$ distribution for the fake-enhanced control regions.
% 
...
% For the fake predictions, a systematic uncertainty derived for the signal region is assumed.
    ...
    \begin{tabular}{ l | r r r r r r r }
        \hline
        Region 		& Fakes 			& Prompt 	& Charge Flip 			& W$\gamma$ 		& Total Pred 			& Data		&  Agreement($\sigma$) \\
            \hline

        SS\_medium++ 	& $ 111.04 \pm 27.4 $	&	$ 2.9 \pm 0.5 $	& $ 72.46 \pm 16.75 $		& $ 8.78 \pm 2.3 $	& $ 195.18 \pm 32.2 $		& $ 217 \pm 15 $&  -0.62\\
        SS\_InterIso 	& $ 252.9 \pm 133.64 $	& $ 1.23 \pm 0.3 $	& $ 29.07 \pm 10.1 $		& $ 0.27 \pm 0.28 $	& $ 283.47 \pm 134.02 $		& $285 \pm 17 $	&  -0.01\\
        SS\_sublInterIso& $ 519.21 \pm 120.72 $ & $ 32.88 \pm 2.14 $	& $ 52.69 \pm 14.87 $		& $ 17.64 \pm 4.32 $	& $ 622.42 \pm 121.72 $		& $574 \pm 24 $ &  0.39\\
        SS\_leadInterIso& $ 154.97 \pm 58.67 $  & $ 13.28 \pm 1.21 $	& $ 15.96 \pm 7.5 $		& $ 5.12 \pm 1.72 $	& $ 189.33 \pm 59.19 $		& $ 224 \pm 15 $&  -0.57\\
