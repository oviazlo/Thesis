...
The LHC reported the first 13 TeV pp collision in May of 2015
% TODO make this cite.
% ~\cite{publicPlots} 
and these were recorded by ATLAS and other LHC 
experiments. 
...
[TODO] improved lumi 7 TeV paper:~\cite{Aad:2013ucp}

% TODO change ``process of interest'' to ``inelastic collisions''
The rate of the process of the interest occurring at the collider experiment can be expressed as
\begin{equation}
R_{process} = \mathscr{L} \sigma_{process}
\label{eq:simpleLumi}
\end{equation}
where $\sigma_{process}$ is a cross section of the process and $\mathscr{L}$ - luminosity delivered by the collider which doesn't depend 
on the type of the process itself. To express number of events where process of interest took place one can integrate \EquationRef{eq:simpleLumi} 
over the data-taking period
\begin{equation}
N_{process} = \int_{t_{1}}^{t_{2}} \mathscr{L} \sigma_{process} dt
\label{eq:simpleLumi2}
\end{equation}
The usual quantity of interest are cross section of the process, which can be calculated by counting number of events where process took place 
occurred over some period and by knowing value of luminosity. If one want to measure cross section for some other process the approach will be 
the same where luminosity will be one of the needed inputs for any type of studied process.
That's why precision measurements of the luminosity is important task for the experiment.
 At the LHC collider every bunch crossing a few pp interaction can occur. Number of the interaction can vary from bunch crossing to by 
 bunch crossing and by assumption it is distributed by the Poisson law with mean $\mu$ - average number of interaction per bunch crossing.
Taking into account revolution frequency of the LHC, $f_{LHC}$, \EquationRef{eq:simpleLumi2} will have a look like
\begin{equation}
\mathscr{L} = \dfrac{f_{LHC} \mu}{\sigma_{process}}
\end{equation}
But there are many bunches in the ring at the same moment and typically luminosity is measured for each bunch crossing individually in 
the experiments which 
\begin{equation}
\mathscr{L} = \dfrac{f_{LHC}}{\sigma_{process}} \sum_{j=1}^{n_{b}} \mu_{j}
\label{eq:lumi_bunch_sum}
\end{equation}
But in reality one can only measure a visible number of interactions ($\mu_{vis}$) which is number of interaction times acceptance and 
efficiency of the method ($\mu_{vis} = \varepsilon \mu$).
In the same way it's possible to express visible cross section ($\sigma_{vis}$) which is cross section of the process time acceptance and 
efficiency of the used luminosity method ($\sigma_{vis} = \varepsilon \sigma_{process}$).
\EquationRef{eq:lumi_bunch_sum} will transform to:
\begin{equation}
\mathscr{L} = \dfrac{f_{LHC}}{\sigma_{vis}} \sum_{j=1}^{n_{b}} \mu_{j}^{vis}
\label{eq:lumi_bunch_sum_visible}
\end{equation}

So to measure luminosity one have to measure a visible number of interaction ($\mu_{vis}$) and visible cross section ($\sigma_{vis}$). 
Former variables for each bunch crossing can be measured by the LUCID detector while latter is measured in the special runs which 
will be described in \SectionRef{subsec:alfa_run}. A period over which luminosity is measured is called luminosity block (LB) and 
typically is equal for one minute in the ATLAS experiment. 

LUCID can measure luminosity separately for all possible bunch slots (LHC has 3564 of them) but only slots which really contain 
bunches are taken into account (maximum of 2808 bunches in the ring).
% TODO do I need really tell this information (in the sentence above)?
% TODO why LUCID can’t count instanteneous luminosity also in BCID aware way?
LUCID also provide so-called instantaneous luminosity which are measured every second and is used by LHC for beam condition monitoring. 
Due to ??? [ask Carla the reason why ins. Lumi is BCID blind] luminosity is measured for all possible bunch slots ???.


Luminosity can be measured in a three different ways with the LUCID detector: by counting number of events, 
number of hits or measuring charge collected by the PMTs. When particle, created in the pp inelastic collision,
penetrate quartz window a clear signal from the PMT can be seen and if it is bigger than a specific threshold 
value we call it a hit. If there is a hit in the at least one PMT in the detector, we call it event. 
LUCID consists from two identical detectors placed from both sides (side A and side C) of the interaction point. 
If there is at least one hit in the side A (side C) detector, ``Event ORA'' (``Event ORC'') take place. ``Event OR'' 
require at least one hit in either detector while ``Event AND'' require at least one hit in both side A and side C 
detectors simultaneously. These four combinations are used in the same way in a hit counting algorithms where number 
PMT hits are counted instead of number of events.
...
The only unknown component in \EquationRef{eq:lumi_bunch_sum_visible} is visible inelastic cross section $\sigma^{vis}$.
This cross section is measured in special beam-separation runs, called van der Meer runs, 
where the absolute luminosity can be measured directly from the beam parameters ~\cite{vanderMeer:296752,Rubbia:1025746}.
Absolute luminosity for centered bunches with respect to each other can be expressed 
with transverse proton density functions of the two beams ($\rho_{1}(x,y)$ and $\rho_{2}(x,y)$) as:
\begin{equation}
\mathscr{L} = f_{LHC} n_{p1} n_{p2} \int \rho_{1}(x,y) \rho_{2}(x,y) dx dy
\label{eq:lumi_vs_intensity}
\end{equation}
where $n_{p1}$ and $n_{p2}$ are number of protons in each of the two colliding bunches.

Number of protons in the bunches (in other words beam currents) are measured by the special direct current (DC) transformers: 
pair of DC current transformers (DCCT) and pair of fast beam current transformers (FBCT). 
DCCT is used to measure total beam current while FBCT is used tomeasure relative intensity of separate bunches.
...
By doing such scan in the both directions and measuring interaction rate as a function of beam shift one can obtain 
two scan curves (with curve widths $\Sigma_{x}$ and $\Sigma_{y}$), one of such curves, normalized to number of protons in the beams, 
...
By comparing \EquationRef{eq:lumi_bunch_sum_visible} and \EquationRef{eq:lumi_vdm} one can show that visible cross section equal to:
...
And by recalling definition of the visible cross section one can express efficiency of luminosity algorithm as:
...
In \TableRef{tab:sigma_vis_efficiency} measured visible cross sections and efficiencies for a few LUCID luminosity algorithms are shown.
Last column in the table contain maximum possible number of interaction which algorithms can deal with without saturation.
These number were calculated for values $\mu_{vis} = 10$.
All counting can start to saturate if number of interaction became large enough. As example let's take a look on ``Event OR'' algorithm.
If in the every bunch crossing at least one hit will be present in the detector (which correspond to ``Event OR'' definition)
ratio $\frac{N_{OR}}{N_{BC}}$ will become equal to 1 and it will be impossible to use \EquationRef{eq:log???} to calculate 
number of interaction $\mu$.
