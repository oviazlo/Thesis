...
Cherenkov light is also produced in quartz fiber bundles that runs parallel to the beamline and that are coupled to the PMTs (see \FigureRef{fig:LucidDrawing} left).
...
interval that corresponds to a LHC bunch crossing. \FigureRef{fig:pulseShape} shows a typical PMT signal shape in 
...
Every LUCROD board receives information from PMTs situated in one of the two detectors.
...
PMTs which are coupled with Bi-207 radioactive sources constantly see signals of radioactive decays which are treated as a background
...
In \SectionRef{subsec:calibPerformance} the calibration strategy during the  2015 and 2016 data-taking periods is discussed.
...
To meet all these requirements a radial design was proposed with a PIN-diode facing three LEDs and located aligned with the LEDs axis. Fibers surround the PIN-diode 
...
The setup was the same as shown in \FigureRef{fig:AngularMeasurementSetup}. The distance between the LED and the PIN-diode 
...
Measurements were repeated three times for each filter configuration. The results of the measurements are shown in \TableRef{tab:FilterChoice}.
...
In order to handle the laser light, a new diffuser was made, as shown in \FigureRef{fig:laserDiffuserSchematics}.
...
% \includegraphics[width=.6\textwidth]{LUCID/laserDiffuserAirGap_v2.pdf}
  \begin{overpic}[width=.6\textwidth]{LUCID/laserDiffuserAirGap_v2.pdf}
    \put (5,62) {\crule[white]{0.5\textwidth}{0.04\textwidth}}
  \end{overpic}
...
The light in the fiber undergoes multiple total internal reflections in the interface between the fiber core and the cladding. 
...
The fiber used to deliver the laser light has a numerical aperture of 0.22, which corresponds to a 3.1 mm air gap needed to cover the 2 mm surface of the diffuser by a light spot (according to \EquationRef{eq:numericalApperture}).
...
Pairs which include one fiber in the central region are called central fibers and marked orange in \FigureRef{fig:laserDiffuserMapping}.
...
The measurements were done with different air gap distances for fiber pairs from each category. 
...
(shown in \FigureRef{fig:pulseheight_Bi207}) corresponds to approximately 30 photoelectrons~\cite{Alberghi:2016tad}.
...
The size of the source is larger than the size of the PMT quartz window (25 mm compared to 10 mm respectively) which led to an observed dependence of the shape of the charge and pulseheight distributions (such as in \FigureRef{fig:pulseheight_Bi207}) on the relative position of the source 
...
In order to perform any temperature tests one has to have a reliable method to measure and control temperature in the testing area.
...
The shape of the digitized pulses from a Bi-207 source is shown in \FigureRef{fig:bi207DigitizedPulse}. 
...
The mean of the charge distribution was calculated for each measurement and is shown as a function of time with red points in \FigureRef{fig:fiberBakeOut}.
...
The LUCID detector was equipped with 18 temperature sensors per side which were installed (see \FigureRef{fig:TempProbes1}
...
The temperature from the sensor placed in the cable tray and at the beampipe flange during the beampipe bake-out is shown in \FigureRef{fig:NEG_ctflangeA}.
...
% \includegraphics[width=.8\textwidth]{LUCID/NEG_ctflangeA_v2.png}
  \begin{overpic}[width=.8\textwidth]{LUCID/NEG_ctflangeA_v2.png}
    \put (92,-1) {Date}
    \put (-5,30) {\rotatebox{90}{Temperature [\degree~C]}}
  \end{overpic}
...
The temperature during the data-taking periods in 2015 and 2016 are shown in \FigureRef{fig:tempTrendingPlot}. 
...
The LUCID detector can measure the luminosity in three different ways: 
...
As one can see from \EquationRef{eq:poissonProb_EventAND}, there is no analytical solution for $\mu$ for ``Event AND'' algorithms, and it can only be solved numerically. 
...
The number of protons in the bunches (which is equivalent to the beam current) is measured a pair of DC current transformers (DCCT) and a pair of fast beam current transformers (FBCT). 
...
% The last column in the table contain the maximum possible number of interactions,
% $\mu_{max}$, which an algorithm can deal with without saturation.
% 
% $\mu_{max}$, at which the statistical error of logarithm calculation as in \EquationRef{eq:logFormula} starts to exceed 2$\%$.
% 
% At these values, the statistical error of luminosity measurements starts to exceed 2$\%$.
...
% 
% TODO proofread paragraph below:
...
These values are shown in the last column in \TableRef{tab:sigma_vis_efficiency}.
...
In \FigureRef{fig:peakMuByFill} one can see that the maximum number of interactions per beam crossing for some physics runs in 2016 was higher than 40-45. At this values, the statistical error of the BI$\_$OR$\_$A algorithm exceeds 2$\%$, which is too large of an errors for the physics runs.
This is why it was decided to switch to the ``HITOR'' algorithm, which counts the number of hits in all PMTs per event.
It saturates on a similar level as the counting algorithm for a single tube (e.g. ``Bi$\_$OR$\_$C9'' in \TableRef{tab:sigma_vis_efficiency}).
...
In \FigureRef{fig:hv_trending_plot_2016} the high voltage change for Bismuth PMTs during 2016 is shown. 
...
%   \includegraphics[width=\linewidth]{LUCID/LumiVsTime_preliminary_v2.pdf}
  \begin{overpic}[width=\linewidth]{LUCID/LumiVsTime_preliminary_v2.pdf}
    \put (80,3) {\crule[white]{0.25\textwidth}{0.05\textwidth}}
    \put (5,25) {\crule[white]{0.05\textwidth}{0.4\textwidth}}
    \put (71,4) {\scriptsize Time [minutes]}
    \put (9,7) {\rotatebox{90}{\scriptsize Average number of pp-interactions}}
  \end{overpic}
  \label{fig:sub3}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
...
%   \includegraphics[width=\linewidth]{LUCID/DeviationSubsystems_preliminary_v2.pdf}
  \begin{overpic}[width=\linewidth]{LUCID/DeviationSubsystems_preliminary_v2.pdf}
    \put (80,3) {\crule[white]{0.25\textwidth}{0.05\textwidth}}
    \put (71,4) {\scriptsize Time [minutes]}
    \put (5,25) {\crule[white]{0.05\textwidth}{0.4\textwidth}}
    \put (9,20) {\rotatebox{90}{\tiny $<\mu_{Algo}>/<\mu_{LUCID}> -1 [\%]$}}
  \end{overpic}
  \label{fig:sub4}
\end{subfigure}
...
The effect of the Bismuth source during vdM scans is shown in \FigureRef{fig:vdmScanCurve}. The background is on a permille level compared to the signal.
...
As can be seen from \FigureRef{fig:afterglow} the maximum rate of the afterglow background for the BI$\_$OR counting algorithm is at the level of 3$\times10^{-5}$. 
...
After each physics run the high voltage for the PMTs is changed in order to compensate for the decrease of the PMT gain (see \SectionRef{sec:pmtGainMonitoringSystem}).
...
The mean of the integrated pulse charge distribution as a function of time is shown in \FigureRef{fig:pmt_gain_profile}. Every point in the plot corresponds to one LED calibration.
...
% TODO not todo, but just a remark about factorisation corrections:
% Factorisation correction is estimated by using the information of the beam spot position to model beams with a correlated triple Gaussian functions. Using information about the modelled beams on can theoreticaly calculate luminosity and compare it with measured in the vdM scan. The difference between these values are taken as systematic uncertainty.
...
The list of all systematic uncertainties in the 2015 luminosity measurements with LUCID is shown in \TableRef{tab:lucid_syst_errors}.
...
As described above, luminosity counting algorithms suffer from a number of unwanted effects. Most of these effects were discovered during the Run-1 operation and the design of the new LUCID detector for Run-2 was aimed to minimize these effects but they still can be observed with the new detector.
...
This is why a lot of effort was invested into development of a precise monitoring system with several independent methods, as described in \SectionRef{sec:pmtGainMonitoringSystem}.
...
However, PMTs become more stable the more they are exposed to luminosity. This can be seen in \FigureRef{fig:hv_vs_cumLumi}, where one can see that the high voltage corrections
