...
The last column in the table contain the maximum possible number of interactions which an algorithm can deal with without saturation.
...
%     BI$\_$AND & 6.38 & 8.0 & 125 \\
  ...
  $\mu_{max}$ has been calculated from $\varepsilon\mu_{max} = 10$.}
...
After investigation it was found that BCM detector significantly underestimate luminosity for bunch trains with 25 ns distance between bunches
and have difficulty to cope with such small distance between bunches.
...
\toAsk[in lumi note run-ro-run stab. uncert. is 1.2$\%$,]

\toAsk[while caption to public plot in \FigureRef{fig:runByRunStability} was stated 2.5$\%$?]
...
The relative variations in measurements of all ATLAS luminometer was found to be within $\pm$ 2.5$\%$ during the year.
\begin{figure}
\centering
...
Another point of interest are FIB and MOD PMTs. FIB PMTs are protected by the shielding and Cherenkov light are produced and delivered by quartz fibers bundles. 
MOD PMTs have reduced quartz window size to 7 mm in diameter. 
Performance of these PMTs are different from VDM and BI PMTs and additional detailed studies are needed to understand their potential benefits to be used for luminosity measurements.

% Another critical problem for the charge method is the afterglow background, which was described for counting algorithms in \SectionRef{subsec:backgrounds}.
% For 25 ns physics runs this background can reach 3$\%$, which make it very important to develop a precise subtraction procedure of the background.
