...
The faster drift velocity of electrons in the Argon leads to a shorter drift time. Since time is digitized in bins of 3.125~ns, shorter drift time will have larger digitization uncertainty. Thus Argon gas will cause a worse position resolution, which can be observed in \FigureRef{fig:resFit}. Worse position residuals means that the momentum resolution will also become worse according to
...
The fractions of precision hits to total number of hits are shown in \FigureRef{fig:precHitFracPerTrack}. A hit is considered to be a precision
