...
The chapter consists of three sections. In \SectionRef{sec:TRT_intro} overview of the detector running conditions is described. In \SectionRef{sec:trt_straw_hw} the general TRT description is presented with emphasis on description of the drift tubes, used gas and front-end electronics.
...
\SectionRef{sec:trt_tracking_perf} describes performance study of the TRT regarding hit and track parameters with focus on active gas mixture, and particularly the comparison of 
Xenon- and Argon-based mixtures.
In \SectionRef{sec:trt_summary} conslusion of the Argon studies are discussed.
...
for the heavy leaking TRT modules. In \FigureRef{fig:mixed_condition_2015_2016} gas configurations of the TRT used in years 2015 and 2016 are shown, demonstrating that the problem is gradually developing with time.
...
In order to have well controlled electron avalanches in the straw, one has to add another gas component, a quencher. Tha main purpose of quencher is to absorb UV photons created in the avalanche process and prevent secondary avalanches, which can lead to an early breakdown.
...
The operating gas gain for straws was chosen to be 2.5 $\cdot$ 10$^4$~\cite{ID_TDR_vol1} and it has to be the same for all straws. It corresponds to 1530~V for the Xenon mixture, as shown in \FigureRef{fig:gas_gain}.
...
\begin{itemize}
 \item Collision time with respect to the edge of the LHC clock. Length of the LHC bunches is typically 1.1-1.2 ns, and collisions are happening along the whole bunch length.\\ \toDo[ask Alex about this!]
...
More detailed description of the ATLAS detector simulation can be find in ref.~\cite{Aad:2010ah}.
...
A very crucial part of the digitization is to model the drift time of the primary electrons which appeared after the interaction of the particle with the gas, to the anode wire, because the drift time is directly related to the drift radius, defined as position of the hit with respect to the straw wire.
...
\EquationRef{eq:momentum_resolution}. Nevertheless, difference between Argon and Xenon is small.
