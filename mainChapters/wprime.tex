...
In the looser muon sample (left-hand plots) the multijet background is significantly enhanced which gives a handle to check the ``fake'' background estimate. 
...
For comparison, the mass limits for three ATLAS analyses done with 8 and 13 TeV data are shown in \TableRef{tab:Wprime_limit_vs_years_ATLAS} as well as the results from the CMS collaboration shown in \TableRef{tab:Wprime_limit_vs_years_CMS}. Measured mass limits are compatible between the experiments.
...
Thus, a sensitivity study of the signal selection for different DM models has been done (see \AppendixRef{app:monoW}).
