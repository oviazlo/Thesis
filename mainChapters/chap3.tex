...
LUCID group published paper with description of choice and characterization of photo multipliers for the new LUCID detector for Run-2~\cite{Alberghi:2016tad}.
...
Block diagram of LUCROD is shown in \FigureRef{fig:LUCROD_schematics}.
...
Input signal are amplified with low noise amplifier with amplification factor of 14, as described in \SectionRef{subsec:LUCIDElectronics} 
...
This fact lead to the idea that it is needed to suppress amount of light which goes to the fibers while keep the same amount for PIN-diode.
...
In order to handle laser light new diffuser was introduced, schematic of which is shown in \FigureRef{fig:laserDiffuserSchematics}.
...
oscilloscope in ATLAS experimental hall. Measurement results are shown at \FigureRef{fig:laserDiffuserDistanceTest}. PMT 1 represent PMTs with central fiber 
...
All measurements in the lab was done with radioactive source enclosed in circular case schematic of which is shown in \FigureRef{fig:Bi207_case} 
...
New material was chosen to minimize the induced radioactivity of the beampipe (see \FigureRef{fig:metalSupportAndTempProbes} (left)).
...
High ambient temperature inside the box was reached with help of thermogun, which was blowing hot air inside the box constantly.
